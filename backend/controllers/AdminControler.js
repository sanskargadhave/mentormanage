...
const AdminLogin = async (req, resp) => {
  try {
    ...
    if (!admin) {
      return resp.status(404).json({ success: false, message: "Admin Not Found" });
    }

    const match = await bcrypt.compare(password, admin.password);
    if (!match) {
      return resp.status(401).json({ success: false, essage: "Password Doesn't Match" });
    }

    resp.status(200).json({ success: true, message: "Login Success.... Welcome Admin", admin });
...
const UserCounts = async (req, resp) => {
  try {
    ...
    resp.status(200).json({ success: true, totalStudents: totalStudents, totalMentors: totalMentors, totalteachers: totalteachers });
  }
  catch (err) {
    resp.status(500).json({ success: false, message: err.message });
  }
...
const DashboardAnalysis = async (req, resp) => {
  try {
    ...
    if (!semester) return resp.status(404).json({ success: false, message: "No active semester found." });
    ...
    resp.status(200).json({success:true,datas:data})
  }
  catch (err) {
    resp.status(500).json({ success:false,message: err.message });
  }
